...
Button::Button(int pin)
    : _pin(pin), _startTime(0), _wasPressed(false), _waitingRelease(false) {}
...
unsigned long Button::get_duration() {
    ...
    if (!isPressed && (_wasPressed || _waitingRelease)) {
        unsigned long duration =
            0; // Needed local variable to restart _startTime
        ...
        // Otherwise, it was a release coming from a waitingRelease, so the
        // duration will be 0, and it stops being on _waitingRelease state
        _wasPressed     = false;
    ...
    if (isPressed && _wasPressed &&
        (millis() - _startTime > config.long_pulse)) {
        _wasPressed     = false;
...
void Button::check() {
    ...
    if (duration > 50)
        handle_button_action(_pin, duration);
}
